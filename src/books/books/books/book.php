 ...
 * @since    2.0
 */
class BooksApiResourceBook extends ApiResource
{
	/**
	...
	private function sendEmail($formData){
		...
		$recipient = array('bihayi6606@confmin.com' );

		$mailer->addRecipient($recipient);
		$body   = "Book titled \"".$formData['title']."\" Book Has been created";
		$mailer->setSubject('JComp Books');
		$mailer->setBody($body);
		$send = $mailer->Send();
		/* if ( $send !== true ) {
	...
	private function retriveBook($xidentifier, $id)
	{
		...
		$db = JFactory::getDbo();
		$query = $db->getQuery(true)
			->select($db->quoteName(array('a.id', 'a.booktitle', 'a.bookpublisher','a.bookprice')))
			->from($db->quoteName('#__books','a'))
			// ->join('INNER', $db->quoteName('#__book_authors', 'b') . ' ON ' . $db->quoteName('a.id') . ' = ' . $db->quoteName('b.book_id'))
			->where($db->quoteName($xidentifier) . ' = ' . $db->quote($id));
		$db->setQuery($query, 0, 1);
